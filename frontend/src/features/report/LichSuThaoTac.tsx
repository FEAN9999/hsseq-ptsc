...
export function LichSuThaoTac({ id, chuyen }: { id: number; chuyen: ChuyenTrangThai[] }) {
  ...
  if (ds.data !== undefined && ds.data.length === 0) {
    return (
      <p className="rounded-xl border border-border bg-card px-4 py-8 text-center text-sm text-secondary-foreground">
  ...
  return (
    <ol className="m-0 list-none rounded-xl border border-border bg-card p-0 [&>li:last-child]:border-b-0">
      {(ds.data ?? []).map((d) => {
